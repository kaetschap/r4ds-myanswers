...
1.  What time of day should you fly if you want to avoid delays as much
    ...
    ```{r}
    flights %>%
      mutate(dep_h = floor(dep_time/100)) %>%
      group_by(dep_h) %>%
      summarize(mean_arr_delay = mean(arr_delay, na.rm = T)) %>%
      arrange(mean_arr_delay)
    ```
    
    The smallest delays occur between 4 and 5:59, where flights are over eight minutes early to arrive. To avoid delays it is sufficient to fly between 4 and 10:59 am.
